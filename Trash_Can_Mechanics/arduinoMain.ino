...
void loop() {
  ...
  if (Serial.available() > 0) { // Check for data on serial port (material type result from AI)
    checkForNewWeight = true; // Enable checking for new weight
    int data = Serial.read(); // Read incoming data on serial port
    
    // Based on material type, operate
    switch (data) {
      case '0': // Move to trash
      ...
      case '2':
        ...
        */
        ...
        for(int x = 0; x < 165; x++) { // Move stepper 1/3rd of the way clockwise
          digitalWrite(stepPin,LOW); 
          delayMicroseconds(5000); 
          digitalWrite(stepPin,HIGH); 
          delayMicroseconds(5000); 
